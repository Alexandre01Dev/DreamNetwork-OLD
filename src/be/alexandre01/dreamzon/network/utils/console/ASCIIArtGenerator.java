...
public class ASCIIArtGenerator {
    ...
    public enum ASCIIArtFont {
        ...
        ART_FONT_MONO("Monospaced"),ART_FONT_SERIF("Serif"), ART_FONT_SANS_SERIF("SansSerif"),ART_FONT_NEW("Courier New");
    ...
    public static void main(String[] args) throws Exception {
        ...
        graphics.drawString("DreamNetwork", 2, 20);
        ImageIO.write(image, "png", new File("text.png"));

        for (int y = 0; y < 32; y++) {
            StringBuilder sb = new StringBuilder();
            for (int x = 0; x < 144; x++)
                sb.append(image.getRGB(x, y) == -16777216 ? " " : image.getRGB(x, y) == -1 ? "1" : "0");

            if (sb.toString().trim().isEmpty()) continue;
            System.out.println(Colors.ANSI_GREEN()+sb);
        }

        ASCIIArtGenerator artGen = new ASCIIArtGenerator();
        System.out.println();
        artGen.printTextArt("DreamNetwork", ASCIIArtGenerator.ART_SIZE_SMALL);
        System.out.println();

        System.out.println();

        artGen.printTextArt("DreamNetwork", 20, ASCIIArtFont.ART_FONT_NEW,"o");
        artGen.printTextArt("Ver. 1.0", 12, ASCIIArtFont.ART_FONT_NEW,"i");
        System.out.println();
